...
# def table(request):
#     names = models.Test.objects.all()
#     return  render_to_response("Searh-result.html",locals())
...
def result(request):
    ...
    #list = search.search1(mes)['_source']
    #print(list)
    list2 = []
    if end!="" or start !="":
...
#         author = "Author: " + mes.get('Author')
#     if "date" in mes:
#         date = "Date weight: " + mes.get('date')
#     if "cited" in mes:
#         cited = "Cited weight: " + mes.get('cited')
#     if "start" in mes:
#         start = mes.get('start')
#     if "end" in mes:
#         end = mes.get('end')
